...
map = File.read('input.txt').rstrip.split("\n").map(&:chars)
@elves = {}
map.each_with_index do |line, y|
  line.each_with_index { |char, x| @elves[to_pos(x, y)] = true if char == '#' }
end
...
begin
  ...
  blocked_proposals.each { |pos| new_elves.delete(pos) }
  @elves = new_elves
  ...
  if rounds == 10
    ...
    puts "part1 #{(max_y - min_y + 1)} part2 #{(max_x - min_x + 1)} part 3 #{@elves.count}"
